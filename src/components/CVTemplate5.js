...
const CVTemplate5 = forwardRef(({ data, color }, ref) => {
  const {
    name,
    title,
    photo,
  ...
  return (
    <div ref={ref} className="min-h-screen w-full">
      {/* Header */}
      <div
    className={`flex flex-col lg:flex-row items-center justify-between gap-6 py-6 px-6 ${textColor}`}
      style={{ background: 'linear-gradient(to right, #4b6cb7, #182848)' }}
    >
        {photo && (
          <img src={photo} alt="Profile" className="w-32 h-32 lg:flex-row rounded-full object-cover border-4 border-white" />
        )}
